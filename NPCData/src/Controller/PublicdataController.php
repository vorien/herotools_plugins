 ...
 *
 */
class PublicdataController extends AppController {

	public function initialize() {
		Parent::initialize();
		$this->viewBuilder()->layout('Vorien/NPCData.public');
	}

	public function addperson() {
		$people = TableRegistry::get('Vorien/NPCData.People');
		...
		if ($this->request->is('post')) {
			...
			if ($people->save($person)) {
				...
				if ($personas->save($persona)) {
					$this->Flash->success(__('The person has been saved and a Public persona has been created.'));
	...
	public function index() {
		...
		$query->innerJoinWith('Agendas', function ($q) {
			return $q->select(['id', 'name'])->where(['Agendas.name' => 'Public']);
		});
		...
		$query->hydrate(false);
		$publiclist = $query->find('all')->toArray();
		$this->set(compact('publiclist'));
	}
...
//		exit;
	...
	public function edit($id = null) {
		...
		$personalities = TableRegistry::get('Personalities')->find('all')->select(['name', 'virtue', 'vice'])->order(['sort_order'])->toArray();
